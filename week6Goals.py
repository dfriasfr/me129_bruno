...
def TurnToNextStreet(direction):
    ...
    while readIRs() == 0 or t1 - t0 < 0.65:
        motor.setvel(0, 16*direction)
        t1 = time.time()
    time.sleep(0.05)
    motor.set(0, 0)
    time.sleep(0.3)
...
if __name__ == "__main__":
    ...
    try:
        ...
        while toExplore:
            target = toExplore[-1]
            goToTarget(target.long, target.lat)
            #face unexplored street
            for i in range(4):
                if target.streets[i] == UNEXPLORED:
                    better_turn(i - heading)
                    break
            #drive down it
            drive()
            toExplore = unexplored()
        ...
        #while some part of the map is unexplored, keep exploring
        #while isFullyExplored() == False:
        #    drive()
        #    print(intersections)
        #goToTarget(0,1)
