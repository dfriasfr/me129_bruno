...
def check():
    ...
    while c < checkTime:
        ...
        #motor.setvel(0, 12)
        s = readIRs()
        if s != 0 and leftState == False:
            leftState = True
        if c == 499:
            #then span right and check
            C = 0
            while C < checkTime*2+150:
                #motor.setvel(0, -12.2)
                s = readIRs()
                if s != 0 and leftState == False:
                    leftState = True
